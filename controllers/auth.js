...
const User = require('../models/User')
 ...
 exports.getLogin = (req, res) => { // Get login page
  ...
  exports.postLogin = (req, res, next) => { // Post login
    ...
    if (!validator.isEmail(req.body.email)) validationErrors.push({ msg: 'Please enter a valid email address.' }) // If email is not valid, push error to array
    if (validator.isEmpty(req.body.password)) validationErrors.push({ msg: 'Password cannot be blank.' }) // If password is empty, push error to array
  
    if (validationErrors.length) { // If validation errors array has errors
  ...
  exports.logout = (req, res) => { // Logout user
    ...
    req.session.destroy((err) => { // Destroy session
      if (err) console.log('Error : Failed to destroy the session during logout.', err) // Log to console
  ...
  exports.postSignup = (req, res, next) => { // Post signup
    const validationErrors = [] // Create validation errors array
    if (!validator.isEmail(req.body.email)) validationErrors.push({ msg: 'Please enter a valid email address.' }) // If email is not valid, push error to array
    if (!validator.isLength(req.body.password, { min: 8 })) validationErrors.push({ msg: 'Password must be at least 8 characters long' }) // If password is less than 8 characters, push error to array
    if (req.body.password !== req.body.confirmPassword) validationErrors.push({ msg: 'Passwords do not match' }) // If passwords do not match, push error to array
    ...
    User.findOne({$or: [ // Find user
      ...
      user.save((err) => { // Save user
        if (err) { return next(err) } // If error, return next with error
        req.logIn(user, (err) => { // Log in user
          if (err) {  // If error, return next with error
